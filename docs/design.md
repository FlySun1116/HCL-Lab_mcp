...
连接建立后先执行最小初始化（例如关闭当前会话分页），结束时尽力回到用户视图。所有设备命令在 **每设备独占锁** 内执行，防止两个 Client 共享状态化 CLI 会话。
...
- 许可证建议：Apache-2.0；`NOTICE` 明确项目为社区互操作项目，与 H3C 无隶属或背书关系。
